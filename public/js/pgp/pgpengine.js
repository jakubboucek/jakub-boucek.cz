var PGPengine = function() {
	...
	function rnTimer()
	{
		...
		window.setTimeout(function(){rnTimer();},randomByte()|128);
	}
	...
	function doEncrypt(keyId,keytyp,pkey,text)
	{
		...
		return '-----BEGIN PGP MESSAGE-----\nVersion: JavascriptPG v2.0\nComment: www.jakub-boucek.cz\n\n'
									+s2r(cp)+'\n='+s2r(crc24(cp))+'\n-----END PGP MESSAGE-----\n';
	}
	...
	function mpi2b(s)
	{
		...
		if(sn < 2)
		{
					console.log('PGPengine error:'+'string too short, not a MPI');
					return 0;
		}
		...
		{
					console.log('PGPengine error:'+'not a MPI, bits='+bits+",len="+len);
					return 0;
		}
	...
	function keyExpansion(key)
	{
			...
			{
				console.log('PGPengine error:'+'Invalid AES key length '+keylen);
				return;
			}
	...
	return {
		...
		encrypt: function(keyId,keytyp,pkey,text) {
			return doEncrypt(keyId,keytyp,pkey,text);
		}
	}
}
